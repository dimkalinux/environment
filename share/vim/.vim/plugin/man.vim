...
func <SID>FindPage(sect, page, mancmd)
  let where = system(a:mancmd." ".s:man_find_arg.' '.s:GetCmdArg(a:sect, a:page))
  if where !~ "^/"
    if matchstr(where, " [^ ]*$") !~ "^ /"
      return 0
    endif
...
func <SID>GetPage(...)
  ...
  if sect != "" && s:FindPage(sect, page, mancmd) == 0
    let sect = ""
  endif
  if s:FindPage(sect, page, mancmd) == 0
    echo "\nCannot find a '".page."'."
    return
  endif
  ...
  silent exec "r!".mancmd." ".s:GetCmdArg(sect, page)." | col -b"
...
endfunc
autocmd BufEnter * call <SID>SetPrg()
